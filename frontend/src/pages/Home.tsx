...
const CATEGORIES = ["All", "Pulses", "Grains", "Vegetables", "Spices", "Dairy", "Oils & Fats"];
...
export default function Home() {
  ...
  function runSimulation() {
    ...
    calculateMutation.mutate({ ingredients: potItems.map(({ ingredientId, quantity, unit }) => ({ ingredient_id: ingredientId, quantity: Number(quantity), unit })), cooking_method: cookingMethod, servings });
  }
        ...
        <section data-testid="hero-section" className="mb-10 grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
          <div className="flex flex-col justify-center py-2 lg:py-8">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-5">
              ...
              <div data-testid="hero-nutrient-count" className="border-l-2 border-[#5b8a62] pl-3"><p className="font-heading text-2xl font-semibold text-[#f7efe9]">08</p><p className="text-[10px] uppercase tracking-[0.15em] text-[#8c7a6d]">nutrients tracked</p></div>
            </div>
          </div>
        ...
        <section data-testid="calculator-workspace" className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(360px,.7fr)]">
          <div className="space-y-6">
            ...
            <Card data-testid="virtual-pot-card" className="glass-panel border-[#3d3028]">
              ...
              <CardContent className="px-5 pb-5 sm:px-6">{potItems.length === 0 ? <div data-testid="empty-pot-state" className="rounded-2xl border border-dashed border-[#59412e] bg-[#18120f]/50 px-5 py-12 text-center"><div className="mx-auto mb-3 w-fit rounded-full bg-[#d95d39]/10 p-3 text-[#d95d39]"><Utensils size={22} /></div><p data-testid="empty-pot-title" className="font-heading text-lg font-semibold text-[#f7efe9]">Your pot is waiting</p><p data-testid="empty-pot-description" className="mx-auto mt-1 max-w-sm text-xs leading-5 text-[#9e8b7b]">Load a recipe above or tap any ingredient to begin your measured cooking simulation.</p></div> : <div data-testid="pot-ingredient-list" className="space-y-2">{potItems.map((item) => { const ingredient = ingredientMap.get(item.ingredientId); if (!ingredient) return null; return <div data-testid={`pot-ingredient-row-${item.instanceId}`} key={item.instanceId} className="grid grid-cols-[minmax(0,1fr)_82px_116px_32px] items-center gap-2 rounded-2xl border border-[#3d3028] bg-[#1c1410]/75 p-3 sm:grid-cols-[minmax(0,1fr)_105px_130px_32px]"><div className="min-w-0"><p data-testid={`pot-ingredient-name-${item.instanceId}`} className="truncate text-sm font-semibold text-[#f7efe9]">{ingredient.name}</p><p data-testid={`pot-ingredient-category-${item.instanceId}`} className="mt-0.5 text-[10px] uppercase tracking-[0.12em] text-[#8c7a6d]">{ingredient.category}</p></div><Input data-testid={`ingredient-quantity-${item.instanceId}-input`} type="number" min="0.1" step="0.1" value={item.quantity} onChange={(event) => updatePotItem(item.instanceId, { quantity: Number(event.target.value) })} className="h-9 border-[#4d392b] bg-[#18120f] px-2 text-right text-sm text-[#f7efe9]" /><select data-testid={`ingredient-unit-${item.instanceId}-select`} aria-label={`Unit for ${ingredient.name}`} value={item.unit} onChange={(event) => updatePotItem(item.instanceId, { unit: event.target.value as Unit })} className="h-9 rounded-md border border-[#4d392b] bg-[#18120f] px-2 text-xs text-[#d4c3b6] outline-none transition-[border-color] duration-200 focus:border-[#e5a93c]">{UNIT_OPTIONS.map((unit) => <option key={unit} value={unit}>{unitLabel(unit)}</option>)}</select><button data-testid={`ingredient-remove-${item.instanceId}-button`} type="button" onClick={() => setPotItems((current) => current.filter((potItem) => potItem.instanceId !== item.instanceId))} className="flex h-8 w-8 items-center justify-center rounded-lg text-[#806f63] transition-[background-color,color] duration-200 hover:bg-[#3a211a] hover:text-[#f28a2e]"><Trash2 size={15} /></button></div>})}</div>}</CardContent>
            </Card>
