      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        ...
        <section data-testid="calculator-workspace" className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(360px,.7fr)]">
          ...
          <aside data-testid="nutrition-dashboard" className="space-y-6 lg:sticky lg:top-6">
            ...
            {result && <Card data-testid="per-serving-nutrition-card" className="glass-panel overflow-hidden border-[#5b8a62]/60"><CardHeader className="border-b border-[#3d3028]/80 px-5 pb-4 pt-5"><div className="flex items-start justify-between gap-3"><div><p data-testid="per-serving-eyebrow" className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8fc596]">Your selected portion</p><CardTitle data-testid="per-serving-title" className="mt-1 font-heading text-xl text-[#f7efe9]">Nutrition per serving</CardTitle><p data-testid="per-serving-description" className="mt-1 text-xs text-[#9e8b7b]">Each value is one equal share of the cooked pot.</p></div><Badge data-testid="per-serving-count-badge" variant="outline" className="border-[#5b8a62]/50 bg-[#5b8a62]/10 text-[#9dcc9f]">1 of {result.servings}</Badge></div></CardHeader><CardContent className="p-5"><div data-testid="per-serving-calories" className="rounded-2xl border border-[#5b8a62]/30 bg-[#182018] p-4"><p data-testid="per-serving-calories-label" className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#8fc596]">Calories</p><p data-testid="per-serving-calories-value" className="mt-1 font-heading text-4xl font-bold text-[#f7efe9]">{prettyNumber(perServing.calories)}<span className="ml-2 text-xs font-medium text-[#9e8b7b]">kcal</span></p></div><div data-testid="per-serving-macro-grid" className="mt-3 grid grid-cols-2 gap-3"><NutritionMetric testId="per-serving-protein" label="Protein" value={prettyNumber(perServing.protein)} suffix="g" accent="#e5a93c" /><NutritionMetric testId="per-serving-carbs" label="Carbs" value={prettyNumber(perServing.carbs)} suffix="g" accent="#d95d39" /><NutritionMetric testId="per-serving-fat" label="Fat" value={prettyNumber(perServing.fat)} suffix="g" accent="#f28a2e" /><NutritionMetric testId="per-serving-fiber" label="Fiber" value={prettyNumber(perServing.fiber)} suffix="g" accent="#5b8a62" /></div><div data-testid="per-serving-micros" className="mt-5 space-y-4 border-t border-[#3d3028] pt-4"><p data-testid="per-serving-micros-title" className="font-heading text-sm font-semibold text-[#f7efe9]">Micronutrients per serving</p><MicroBar testId="per-serving-sodium" label="Sodium" value={perServing.sodium} max={2300} suffix="mg" /><MicroBar testId="per-serving-iron" label="Iron" value={perServing.iron} max={18} suffix="mg" /><MicroBar testId="per-serving-calcium" label="Calcium" value={perServing.calcium} max={1000} suffix="mg" /><MicroBar testId="per-serving-b12" label="Vitamin B12" value={perServing.b12} max={2.4} suffix="mcg" /></div></CardContent></Card>}
